...
def main(det_thresh=0.05, gallery_size=-1, ignore_cam_id=True):
    ...
    for det in all_dets:    # 6112
        ...
        RoI_Align_feats.append(RoI_Align_feat)
    ...
    for probe in probe_set:
        ...
        id = name_id[name]
        det = gallery_det[id]   # 一张图片中多个bbox
        feat = gallery_feat[id]
        # Probe is in the gallery
    ...
    search_performance_calc(gallery_set, probe_set, gallery_det, gallery_feat, probe_feat, det_thresh, gallery_size, ignore_cam_id, RoI_Align_feats)

# @jit(forceobj=True)
def search_performance_calc(gallery_set, probe_set, gallery_det, gallery_feat, probe_feat, det_thresh=0.5, gallery_size=-1, ignore_cam_id=True, RoI_Align_feats=None):

    assert len(gallery_set) == len(gallery_det)
    assert len(gallery_set) == len(gallery_feat)
    assert len(gallery_set) == len(RoI_Align_feats)
    assert len(probe_set) == len(probe_feat)

    gt_roidb = gallery_set
    name_to_det_feat = {}
    for gt, det, feat, roi_feat in zip(gt_roidb, gallery_det, gallery_feat, RoI_Align_feats):
        name = gt['im_name']
        pids = gt['gt_pids']
        cam_id = gt['cam_id']
        scores = det[:, 4].ravel()
        ...
        if len(inds) > 0:
            name_to_det_feat[name] = (det[inds], feat[inds], pids, cam_id, roi_feat[inds])
    ...
    for i in tqdm(range(len(probe_set))):
        ...
        # find image name with the same identity
        gallery_imgs = []
        for x in gt_roidb:
        ...
        for item in gallery_imgs:
            ...
            if gallery_imname not in name_to_det_feat:
                continue
            det, feat_g, _, _, roi_feat = name_to_det_feat[gallery_imname]
            ...
            roi_feats.extend(list(roi_feat))
